...
"En nuestro código todo lo que está en paréntesis representa los parámetros "
"de nuestro init y nuestro algoritmo empieza desde que declaramos el “if”: "

    def __init__(self, config, n, parent=None, action="Initial", cost=0):
        if n * n != len(config) or n < 2:
            raise Exception("La longitud de la configuracion no es correcta!")
"El self debe ser el parámetro del método para acceder a los métodos y atributos de las instancias"
"Esta parte siemplemente es para calcular la longitud de nuesta configuracion en el confi empezamos a pasar los parametros"
"Aqui abajo le estamos dando funcion al parametro"
        self.n = n
        self.cost = cost
        self.parent = parent
        self.action = action
        self.dimension = n
        self.config = config
        self.children = [] "Si toda esta parte se cumple se le añade al nodo hijo, aqui es donde ingresamos los datos"

"Esta parte lo que hace es que busca la coordenadas de cero(Obtener el valor del hijo)"
        for i, item in enumerate(self.config):
            if item == 0:
                self.blank_row = i // self.n
                self.blank_col = i % self.n
                break "Si hay otro numero diferente de cero el codigo no se va a romper o dañar, solo se sequira ejecutando hasta que
                "lo encuentre o lo va a finalizar"

            "El self en estas funciones hace referencia a la instacia de la clase"


    def display(self): 
        "Ya que los datos introducidos seran una secuencia de numeros, es mas apropiado  utilizar range,"
"la misma nos da 10 valores y el ultimo no se cuenta, por lo tanto seria el  parametro vacio que necesitamos"
"Aqui solo decimos que para i en el rango de la clase self con la linea vacia que  representa los datos que vamos 
"a ingresar"

        for i in range(self.n):
            line = [] "aqui introducimos nuestros valores"
            offset = i * self.n "Multiplica nuestra variable i por la instancia de la clase cuando el rango de j "
            for j in range(self.n):
                line.append(self.config[offset + j])"Aqui se representan los datos que vamos a ingresar"
            print(line)
            "Aqui inicia el codigo de nuestro movimientos haciendo uso de la misma instancia  de clase self,"
"hacemos referencia a la forma en lque nuestro move left puede o no moverse, y  los parametros que se le devolveran"
"puzzle dibujado a la hora de que se ejecute"


    def move_left(self):"A nuestro parametro self le decimos como se puede mover a la izquierda"
        if self.blank_col == 0:
            return None
            ...
            "Lo mismo con este y todos los move"
